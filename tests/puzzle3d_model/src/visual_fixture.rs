...
fn write_scenes(out: &mut String, parsed: &ParsedPuzzle3) {
    ...
    if scenes.is_empty() {
        ...
        for (index, scene) in scenes.iter().enumerate() {
            ...
            out.push_str("},\n");
            out.push_str("      \"rules\": [");
            for (rule_index, rule) in scene.rules.iter().enumerate() {
                if rule_index > 0 {
                    out.push_str(", ");
                }
                write_scene_rule_call_json(out, rule);
            }
            out.push_str("],\n");
        ...
        SceneComponent::Frame(frame) if frame.kind == "puzzle3" => {
            ...
            if !frame.inputs.is_empty() {
                ...
                for (index, input) in frame.inputs.iter().enumerate() {
                    ...
                    write_component_input_json(out, input);
                }
                out.push('}');
            }
...
fn write_scene_rule_call_json(out: &mut String, rule: &SceneRuleCall) {
    write!(
        ...
        json_string(&rule.target),
        json_string(&rule.rule)
    )
    .unwrap();
    if !rule.input_map.is_empty() {
        out.push_str(", \"inputMap\": {");
        for (index, entry) in rule.input_map.iter().enumerate() {
            if index > 0 {
                out.push_str(", ");
            }
            write_input_map_entry_json(out, entry);
        }
        out.push('}');
    }
    out.push_str(" }");
