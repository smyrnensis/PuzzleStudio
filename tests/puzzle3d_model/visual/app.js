...
async function loadSnapshot() {
  try {
    if (window.Puzzle3DFixture) {
      snapshot = normalizeSnapshot(window.Puzzle3DFixture);
    } else {
  ...
  currentSceneName = initialSceneName(snapshot);
...
function renderScene() {
  ...
  if (componentEmbedMode && embeddedPuzzle3Component) {
  ...
  const rootComponent = {
    kind: "column",
    children: components,
    layout: { gap: 0, align: { x: "center", y: "center" } },
  };
