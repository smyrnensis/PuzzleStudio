...
let levelEditorApplication = null;
let levelEditorApplicationRevision = 0;
...
function activateSource(module, source) {
  ...
  levelEditorApplication = null;
  levelEditorApplicationRevision = 0;
  return revision;
}
...
async function queryAnalysis(request) {
  ...
  switch (request.method) {
    ...
    case "levelEditorBundle": {
      const application = requiredFunction(
        module,
        "open_active_source_analysis_level_editor_application",
      )(revision, String(request.modelName || ""));
      const manifest = application.handle({ type: "manifest" });
      const authoringModelProjection = application.handle({ type: "authoringModelProjection" });
      const objects = Array.isArray(manifest.objects) ? manifest.objects : [];
      const visuals = objects.map((object) => application.handle({
        type: "visual",
        objectId: Number(object?.id),
      }));
      const snapshots = (Array.isArray(manifest.levels) ? manifest.levels : []).map((level) => ({
        levelIndex: Number(level?.levelIndex),
        snapshot: application.handle({
          type: "snapshot",
          levelIndex: Number(level?.levelIndex),
        }),
      }));
      levelEditorApplication = application;
      levelEditorApplicationRevision = revision;
      return { revision, manifest, authoringModelProjection, visuals, snapshots };
    }
    case "levelEditorRequest": {
      if (!levelEditorApplication || levelEditorApplicationRevision !== revision) {
        throw new Error(`Level editor application is stale for revision ${revision}`);
      }
      return levelEditorApplication.handle(request.applicationRequest || {});
    }
    default:
