...
function level3dLayerCommandActive(context) {
  return level3dCommandActive(context) && !level3dPlaytestActive;
}
...
function level3dSliceCommandAvailable(context) {
  ...
  return !target?.closest("[data-level3d-slice-scrub]");
}
...
function runVisualTransformCommand(context, transform) {
  const dimension = visualCommandDimension(context);
  const transforms = dimension === "3d"
    ? {
        "rotate-left": rotateVisual3dPlaneLeft,
        "rotate-right": rotateVisual3dPlaneRight,
        "flip-horizontal": flipVisual3dPlaneHorizontal,
        "flip-vertical": flipVisual3dPlaneVertical,
      }
    : dimension === "2d"
      ? {
          "rotate-left": rotateVisualLeft,
          "rotate-right": rotateVisualRight,
          "flip-horizontal": flipVisualHorizontal,
          "flip-vertical": flipVisualVertical,
        }
      : null;
...
// Only the declarable composer has a `sounds` declaration, so source-writing
// sound commands stay unavailable while another composer is auditioned.
function soundsCommandCanWriteSource(context) {
  return soundCommandKind(context) !== "music" || soundMusicIsDeclarable();
}
...
function runSoundsAddCommand(context) {
  insertSoundsDefinition(soundCommandKind(context)).catch((error) => {
    console.error(error);
    setStatus(`Could not insert sounds: ${error?.message || error}`, "is-error");
  });
  return true;
}
...
const editorCommandDatabase = [
  ...[
    {format: "png", mode: "visual", selector: "#visualExportPngButton"},
    {format: "gif", mode: "visual", selector: "#visualExportGifButton"},
    {format: "vox", mode: "visual3d", selector: "#visualExportVoxButton"},
  ].map(({format, mode, selector}) => ({
    id: `visual.export.${format}`,
    group: "visual",
    label: `Export ${format.toUpperCase()}`,
    elements: editorCommandElements(selector),
    available: context => editorCommandRouteIs(context, "visual", [mode]) && !visualAssetExportPending,
    run: () => exportVisualAsset(format),
  })),
  {
    id: "workspace.save",
    group: "workspace",
  ...
  },
  {
    ...
    allowTextEntry: true,
    available: (context) =>
      editorCommandRouteIs(context, "sounds") && soundsCommandCanWriteSource(context),
  ...
  },
  {
    id: "sounds.add",
    group: "sounds",
    label: "Add sound to source",
    shortcuts: [{ key: "a" }],
    elements: editorCommandElements("#soundsSfxInsertButton, #soundsMusicInsertButton"),
    available: (context) =>
      editorCommandRouteIs(context, "sounds") && soundsCommandCanWriteSource(context),
    run: runSoundsAddCommand,
  },
  {
  ...
  },
  {
    ...
    label: "Grid",
    shortcuts: [{ key: "g" }],
    elements: editorCommandElements("#level3dLayerPalette .level-grid-button"),
    available: level3dLayerCommandActive,
    run: () => (toggleLevel3dLayerGrid(), true),
  },
  {
    id: "level3d.play",
    group: "level3d",
  ...
  },
  {
    ...
    label: "Clip",
    shortcuts: [{ key: "c" }],
    elements: editorCommandElements(".visual-editor-toolbar .visual-clip-button"),
    available: visualCommandActive,
    run: toggleVisualClipCommand,
  },
  {
    id: "visual.transform.flip-horizontal",
    group: "visual",
    label: "Flip horizontal",
    shortcuts: [{ key: "h" }],
    elements: editorCommandElements("#visualFlipHorizontalButton, #visual3dFlipPlaneHorizontalButton"),
    available: visualCommandActive,
    run: (context) => runVisualTransformCommand(context, "flip-horizontal"),
  },
  {
    id: "visual.transform.flip-vertical",
    group: "visual",
    label: "Flip vertical",
    shortcuts: [{ key: "v" }],
    elements: editorCommandElements("#visualFlipVerticalButton, #visual3dFlipPlaneVerticalButton"),
    available: visualCommandActive,
    run: (context) => runVisualTransformCommand(context, "flip-vertical"),
  },
  {
    id: "visual.transform.rotate-left",
    group: "visual",
    label: "Rotate left",
    shortcuts: [{ key: "q" }],
    elements: editorCommandElements("#visualRotateLeftButton, #visual3dRotatePlaneLeftButton"),
    available: visualCommandActive,
    run: (context) => runVisualTransformCommand(context, "rotate-left"),
  },
  {
    id: "visual.transform.rotate-right",
    group: "visual",
    label: "Rotate right",
    shortcuts: [{ key: "e" }],
    elements: editorCommandElements("#visualRotateRightButton, #visual3dRotatePlaneRightButton"),
    available: visualCommandActive,
    run: (context) => runVisualTransformCommand(context, "rotate-right"),
  },
  {
    id: "visual.edit.copy",
    group: "visual",
  ...
  {
    id: "visual3d.scope.toggle",
    group: "visual",
    label: () => visual3dEditScope() === "all" ? "Edit whole volume" : "Edit current slice",
    shortcuts: [{ key: "s" }],
    elements: editorCommandElements("#visual3dEditScopeButton"),
    available: visual3dCommandActive,
    run: () => (
      setVisual3dEditScope(visual3dEditScope() === "slice" ? "all" : "slice"),
      true
    ),
  },
  {
...
function editorCommandShortcuts(id, element = null) {
  ...
  if (!Array.isArray(shortcuts)) throw new Error(`Editor command ${id} shortcuts must be an array.`);
  return shortcuts;
}
...
function bindEditorCommandElement(element, command) {
  ...
  if (shortcuts.length) {
    ...
    delete element.dataset.shortcuts;
...
function invokeEditorCommand(id, context) {
  ...
  if (result && typeof result.then === "function") {
    result.finally(refreshEditorCommandBindings);
  } else {
