...
| `turn3d.applyRuleGroup` propagation loop | Rule group repetition, `once`, random groups, loop/gosub flow, and command timing are PuzzleScript semantics, not 3D semantics. | Shared rule-group runner or shared rule-group control artifact with dimension-specific match/apply hooks. | In progress: `random` rule group candidate enumeration and selection now uses `src/js/random_rule_groups.js` from both 2D and 3D, pinned by a 2D VM-oracle parity test and 3D turn/E2E tests. Ordinary propagation, loops, and gosub remain to share. |
...
| `compiler.groupRules3D` | Rule grouping is not spatial. | Shared group arrangement/collapse behavior. | 2D and 3D group ordering and grouping are produced by the same code or same tested helper. |
| `rules3d.applyCellReplacement` for non-spatial replacement semantics | Object/movement clear/set, random object, randomdir, rigid bookkeeping, and sfx tracking are 2D semantics. Only target cell addressing is spatial. | Shared replacement helper with board/movement/sfx/random/rigid adapters. | In progress: object/movement clear-set, random object, randomdir replacement, rigid replacement bookkeeping, rigid rollback/resimulation, create/destroy sfx artifact collection, and movement/cantmove sfx collection now use shared helpers or shared artifact contracts with 2D VM-oracle parity and 3D E2E tests. `randomdir` and sound direction masks differ only by spatial direction count/bit table. Broader session artifacts are still pending. |
