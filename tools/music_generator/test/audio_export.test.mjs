...
import { generateFunctionalSong } from "../seeded_music_functional.mjs";
...
assert.equal(songExport.ps, "music music-seed tone=0.62 bpm=106 bars=32 volume=0.7");

const functionalExport = exportMusicLoop(generateFunctionalSong("functional-seed", { height: 0.8, bpm: 110, bars: 16, volume: 0.5 }));
assert.equal(functionalExport.height, 0.8);
assert.equal(functionalExport.brightness, 0.5);
assert.equal(functionalExport.presence, 0.5);
assert.equal(functionalExport.attack, 0.5);
assert.equal(functionalExport.bars, 16);
assert.equal(functionalExport.ps, "music functional-seed height=0.8 bpm=110 bars=16 volume=0.5");
