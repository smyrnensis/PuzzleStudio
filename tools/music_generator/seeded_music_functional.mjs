...
export function generateFunctionalSong(seed, options = {}) {
  ...
  const sectionPlan = markLoopHandoff(buildSectionPlan(rng, form, obligations, bars / 8));
  ...
  for (let bar = 0; bar < bars; bar += 1) {
    ...
    addIdentity(events, sectionObligation(obligations.identity, sectionState), timbres, tonic, scale, chordRoot, bar, localBar, sectionState, seedText);
    addTime(events, sectionObligation(obligations.time, sectionState), timbres, chord, bar, localBar, sectionState, seedText);
    addTone(events, sectionObligation(obligations.tone, sectionState), timbres, chord, bar, localBar, sectionState, seedText);
    addMotion(events, sectionObligation(obligations.motion, sectionState), timbres, tonic, scale, chordRoot, bar, localBar, sectionState, seedText);
    addColor(events, sectionObligation(obligations.color, sectionState), timbres, chord, bar, localBar, sectionState, seedText);
    addBoundary(events, sectionObligation(obligations.boundary, sectionState), timbres, tonic, scale, bar, localBar, sectionState, seedText);
  }
...
function buildSectionPlan(rng, form, obligations, sectionCount = 8) {
  ...
  return trajectory.map((state, index) => realizeSectionState(state, index, rng, form, motif, distantOverrides, foregroundOverrides));
}
...
function buildStateTrajectory(rng, form, sectionCount) {
  ...
  for (let index = 0; index < sectionCount; index += 1) {
    ...
    const noveltyBase = index === 0
      ? 0.08
      : clamp(0.1 + form.contrast * 0.28 + progress * 0.26 + (rng() - 0.5) * 0.34, 0.02, 0.92);
    ...
    const memoryDistance = previous
      ? clamp(previous.memoryDistance * (0.48 + rng() * 0.22) + noveltyBase * 0.62 - closureBase * 0.2, 0, 0.96)
      : 0.06;
    const tension = clamp(0.16 + noveltyBase * 0.5 + memoryDistance * 0.26 + progress * 0.16 - closureBase * 0.22 + (rng() - 0.5) * 0.22, 0.04, 0.94);
...
function realizeSectionState(state, index, rng, form, motif, distantOverrides, foregroundOverrides) {
  ...
  const overrides = carrierOverridesForState(state, form, distantOverrides, foregroundOverrides, rng);
...
function carrierOverridesForState(state, form, distantOverrides, foregroundOverrides, rng) {
  if (state.progress < 0.18 || state.memoryDistance < 0.2 && state.novelty < 0.24) {
    return null;
  }
  if (state.closurePressure > 0.7 && state.density > 0.48) {
  ...
  if (state.memoryDistance > 0.5 || state.novelty > 0.58) {
    return distantOverrides;
  }
  return rng() < 0.42 ? foregroundOverrides : null;
}

function sectionState(index, variant, degreeOffset, progressionShift, identityLevel, motionLevel, colorLevel, boundaryLevel, motifVariant, state, energy, carrierOverrides = null) {
  return {
    name: `section-${index}`,
    index,
    variant,
    motifVariant,
    phraseShape: buildPhraseShape(index, variant, degreeOffset, progressionShift, motifVariant, state),
    carrierOverrides,
    degreeOffset,
    progressionShift,
...
function buildPhraseShape(index, variant, degreeOffset, progressionShift, motifVariant, state) {
  ...
  for (let index = 0; index < 8; index += 1) {
    ...
    const boundary = phraseBoundaryForBar(index, barState, slopeIn, slopeOut, energy, rng);
    const pickup = phrasePickupForBar(index, energy, slopeOut, boundary, rng);
...
function phraseBar(sectionState, localBar) {
  return sectionState.phraseShape?.bars?.[localBar % 8] ?? {
    index: localBar % 8,
    targetCenter: 0,
    heightBias: 0,
...
function addIdentity(events, obligation, timbres, tonic, scale, chordRoot, bar, localBar, sectionState, seedText) {
...
function addTime(events, obligation, timbres, chord, bar, localBar, sectionState, seedText) {
  const barShape = phraseBar(sectionState, localBar);
  ...
  if (obligation.carrier === "thin-pulse") {
...
function addTone(events, obligation, timbres, chord, bar, localBar, sectionState, seedText) {
  const barShape = phraseBar(sectionState, localBar);
  const rng = eventRng(seedText, "tone", obligation.carrier, sectionState, localBar);
...
function addMotion(events, obligation, timbres, tonic, scale, chordRoot, bar, localBar, sectionState, seedText) {
  const barShape = phraseBar(sectionState, localBar);
...
function addColor(events, obligation, timbres, chord, bar, localBar, sectionState, seedText) {
  const barShape = phraseBar(sectionState, localBar);
  const rng = eventRng(seedText, "color", obligation.carrier, sectionState, localBar);
  if (obligation.carrier === "none") {
    return;
  }
  if (obligation.carrier === "air-pad" && barShape.colorAccent) {
  ...
  if (obligation.carrier === "noise-halo" && (barShape.colorAccent || barShape.space > 0.58)) {
  ...
  if (obligation.carrier === "organ-bed" && barShape.toneAnchor) {
  ...
  if (obligation.carrier === "bright-accent" && (barShape.pickup > 0.3 || barShape.boundary > 0.7)) {
...
function addBoundary(events, obligation, timbres, tonic, scale, bar, localBar, sectionState, seedText) {
  const barShape = phraseBar(sectionState, localBar);
  ...
  const rng = eventRng(seedText, "boundary", obligation.carrier, sectionState, localBar);
