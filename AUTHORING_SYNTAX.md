...
- component behavior: lowering 後の component が入力の意味を所有する。`choice` は cursor と confirm、scrollable container は scroll を所有する。`level_menu` は runtime component ではない。
...
body's appearance, determines whether content is a visual, grid, rule, or another
leaf language.
...
nodes. Multiword directives such as `rotate from up`, ASCII visuals, rule
...
`layers` は object 定義から作られる組み込み tag set。名前付き state layer はその名前、匿名 state layer は内部名で展開される。各 layer 名は同じ state layer に属する object group としても登録される。
...
rewrite 内の object/group、schema tag、movement set は同じ binding lifecycle に従う。
左辺の集合 selector は、match した concrete value を捕捉する binding 宣言であり、
右辺の集合 selector はその binding の参照である。右辺だけで集合を独立展開しない。

参照先は、明示した `#label`、同じ selector occurrence、一意な compatible binding の
順で決める。候補が複数なら ambiguous error、左辺に候補がなければ unbound error。
`no` で否定した集合 selector は binding を作らない。複数の独立 binding が必要な場合は
`#1` / `#2` のような rewrite-local label を付ける。

`for value in <set>` はこの match binding とは別で、有限集合を source order で列挙する
lexical expansion である。
...
```txt
[ directions A ] -> [ directions A directions B ]
[ directions#1 A directions#2 B ] -> [ directions#2 A directions#1 B ]
```

最初の例では左辺の movement capture が一意なので、右辺の `A` と `B` は同じ方向を
受け取る。2つ目では `#1` と `#2` が独立した方向を捕捉し、右辺で明示的に交換する。
...
`layers` は位置を持つ object の state layer と、背面から前面への render priority を同じ順序付き宣言から作る canonical authoring block。object / schema を生成できる owner は `layers` だけで、独立した object 宣言 block はない。`visuals` は見た目の resource と object binding だけを所有し、描画順は所有しない。
...
名前付き state layer はそのまま tag selector として使える。たとえば `floor = Goal Button` と書いた後は、rule や legend や condition で `floor` が `Goal Button` の selector として解決される。

右辺の通常名は、未知なら新しい object / schema として作られ、既存の object / schema / group / layer tag ならその selector をその state layer に割り当てる。`!<visual>` は transient animation visual の render source であり、object を作らず state layer にも入らない。同じ priority に通常 object と animation visual を並べられるため、`Box !Box` も有効である。`Box` と `!Box` は別の名前空間を参照する。
...
puzzle 直下の declaration/use block は同じ puzzle scope の catalog に対して解決される。したがって `sounds`、`rules`、`win_conditions`、`legend` などの object selector は、同じ puzzle 内の `layers` が作る最終 catalog を見る。
...
level 文字と表示文字は `levels { legend { ... } }` に書く。

semantic selector は state layer declaration とは別責務なので、canonical では group row を `groups { ... }` の中に集める。

同じセルの同じ state layer には最大 1 object しか入れない。各通常行は一つの state layer と一つの render priority を同時に作る。`!Visual` だけの行は state layer を作らず、animation visual の render priority だけを作る。
...
`merge { ... }` 内の各行は別々の state layer を作るが、描画時には一つの unordered merge priority へ合流する。同じ pixel / voxel に色が存在するときは、透明な sample を除く各 RGBA channel の単純算術平均で合成する。merge 内の行順は合成結果に影響しない。merge は入れ子にできない。

`priority = <direction...>` は `layers` 直下で、異なる cell に属するはみ出し visual の座標比較順を定義する。左から辞書式に比較し、最初に差が出た `<direction>` 側の cell を前に描画する。2D は2軸を1回ずつ、3D は3軸を1回ずつ必須とする。省略時は2Dが `down right`、3Dが `down right front`。
...
`marks` は rule chain の中だけで使う transition-local な一時 fact を宣言する block。mark は `State` に保存されず、level / undo / solver key / renderer には残らない。`wait` で同じ program が中断・再開される間だけ continuation が保持し、transition または lifecycle program の実行が終わるとすべて消える。
...
見出し名は英数字、空白、`_`、`-` からなる名前を lowercase snake_case に正規化し、既存 block 名に対応する場合だけ section として扱う。たとえば `RULES` は `rules`、`ON LEVEL START` は `on_level_start`、`LAYERS` は `layers`、`LEGENDS` は `legend` になる。`TRANSITIONS` は canonical section ではない。未知の見出しは section sugar ではなく通常行として扱われる。
...
once input [ Fire | Wood ] -> [ Fire | Fire ]
once input [ Fire | Grass ] -> [ Fire | Fire ]
}
```
...
方向や selector の内部展開が複数の concrete rule を生成しても、1つの rewrite 行は1つの `once` application boundary のまま。`for d in directions { ... }` は body を方向ごとの別 statement へ展開するため、各 statement がそれぞれの `once` boundary を持つ。
...
`for <binding> in <source...>` は、有限で順序を持つ source の各 item へ statement list を展開する。source には組み込み方向set、`layers`、author-defined tag set、numeric range、または inline value list を使える。
...
`layers` は state layer group 名へ展開されるため、`no s` は展開後のその layer group 全体の不存在条件になる。他の layer の object は禁止しない。animation-only layer は object selector を持たないため、この tag set には入らない。
...
prefix なしでも、複数セル、複数行、ellipsis、または `>` / `<` / `^` / `v` のような相対方向属性を含む pattern は空間方向へ暗黙展開する。rewrite、pattern condition、condition pattern のすべてで同じ規則を使う。
...
2Dでは `up` / `down` / `left` / `right`、3Dでは `horizontal`、つまり `left` / `right` / `front` / `back` の4方向 variant へ lower される。3Dで上下方向も必要な場合は `directions [ ... ]` のように明示する。
...
一つの rule segment で同じ object occurrence に direction rotation と move が発生すると、visual tween と position move は独立した event として renderer に渡る。renderer は両 channel を同じ progress で評価するため、位置移動と回転は同時に進む。`Player:left -> Player:up` のような direction variant の rotation rewrite は tween 対象になる。任意 tag variant の置換は visual 差分だけでは tween 対象にならない。途中に `wait animation` または時間指定の `wait` を置くと event batch が分かれ、resume 後の更新は次の tween になる。
...
```txt
input [ player:color | box:color | ] -> [ | player:color | box:color ]
```
...
`layers` の `each <selector...>` 行は、selector の concrete alternatives をそれぞれ別の通常 layer に展開する。これは collision しない特殊 layer ではない。作られた各 layer は通常どおり state layer であり、宣言順の表示順も持つ。`each` 行に animation visual は書けない。
...
visual のインライン ASCII shape を明示する場合は `shape = { ... }` を使う。
...
外部画像は `Box visuals/box.png` のように selector と画像パスを 1 行に書ける。パスは game folder からの相対パスとして HTML renderer に渡される。
...
HTML renderer が生成する visual 名と CSS class は、object 名の大文字・小文字を保持する。CSS class として危ない区切り文字だけ `-` に置き換える。例: `Player` は `.visual.Player`、`Box:A` は `.visual.Box-A`。
...
level 固有の演出は `message` 自体に level 指定を持たせず、puzzle target が公開する condition または明示した scene state を scene / lifecycle 側の `if` で読む。layout の iterable binding を runtime global として参照しない。

```txt
on_scene_start {
if selected_level == microban_01 {
message hint
}
}

rules {
if selected_level == microban_03 and sokoban.special_clear -> {
...
`level.name` は `for level in levels` の binding 内でのみ level record の `name` field を表す。record 自体は text や level name に暗黙変換されないため、field を明示する。`level.label` や `level.title` の別名、`level.last` / `level.has_next` の暗黙 field はない。

puzzle rule の rewrite effect としても `message "text"` / `message <path>` / `sfx <name>` を書ける。`message` は `standard.message` overlay instance を surface に追加し、`dismiss` component event まで同じ turn の後続 rule、win 判定、lifecycle を停止する。dismiss 後は保存された continuation から再開する。時間待ちが必要な場合は独立した `wait <duration>` を使う。
...
top-level に `puzzle sokoban { ... }` を定義し、同名の `scene sokoban` がなければ、同じ名前の playable scene が自動で追加される。これは `state { puzzle sokoban }`、`layout { sokoban }`、`rules { step sokoban }` を持つ scene と同等で、`goto sokoban(first)` のように直接入れる。`puzzle push3d { ... }` でも同じ規則で `puzzle` window の scene が追加される。モデル block 内に `layout { ... }` を書いた場合は、その `layout` が同名 scene の layout になり、bare `puzzle` / `puzzle` はそのモデル自身の window を意味する。作者が `scene sokoban { ... }` を明示した場合は、その明示 scene が override であり、自動 scene は追加されない。

ゲーム開始時は、top-level の `scene` / `puzzle` をソース順に scene として数え、最初のものを開く。したがって title scene から始める場合は、`scene title { ... }` を `puzzle { ... }` より前に書く。
...
renderer はtext roleとbuttonを既定の`space fit`、`puzzle` / `frame`を既定の`space fill 1`として扱う。`space fill <weight>`は親の主軸に残った空間をweight比で受け取り、`aspect <w> <h>`はslot内の比率だけを定める。containerの`align`はcross axis、`distribute`はmain axisを所有する。文字サイズはlayoutではなくroleとthemeが所有する。
...
Scene layout は `puzzle` を固定 4:3 display として扱う。`puzzle` は可変 window ではなく、その固定 display の内側に 3D visual を描く component である。scene は level の幅、focus object、`zoomscreen` の有無、投影後の見え方を layout 判断に使わない。`zoomscreen` は、親から渡された frame `W x H` と viewport 指定の cell frame `W cells x H cells` から、3D visual が display 内の描画位置と倍率を決める機能である。
...
Canonical な scene layout form は `heading`、`subheading`、`text`、`caption`、`choice`、`button`、`row`、`column`、`box`、`for`、`level_menu`、`menu`。`for` と `level_menu` は authoring 時に消える。4つの text keyword は一つの text component を異なる role で作る。Model window component は `puzzle`。`layout` は component ではなく scene root layout block。`panel` は component keyword ではない。
...
`scene puzzle [name]` は puzzle state を主モデルに持つ playable scene を定義する。`name` を省略すると `playing` になる。scene-local な state slot は puzzle instance を保持し、`layout` は画面配置を意味する。scene-local な puzzle slot を明示しない場合は、`<name>` state slot が暗黙に `puzzle <name>` として用意される。`board` は予約 slot 名ではない。明示した slot がある場合はそれが primary puzzle slot になり、`update <slot>` で現在 input をその puzzle transition に渡せる。
...
`scene level_menu [name]` の typed scene template は読まない。level list は通常 scene の `layout` 内に `level_menu` authoring sugar として置く。`show_index = <true|false>`、`layout = list`、`columns = <n>`、`button ...` は `level_menu { ... }` 内に書く。
...
`columns = <n>` は level item を通常の `row` に分割して n 列に配置する。`layout = list` は通常の縦 list。方向入力と決定は、生成された通常の `choice` と同じ規則に従う。
...
button "Restart" -> playing.restart
button "Title" -> goto title
button level.name -> goto level.puzzle(level.name)
```
...
level list の基本形は、通常の `scene` の `layout` に `level_menu { ... }` を置くこと。これは `column scroll=true`、`for level in levels`、通常の `choice`、typed `goto` effect へ authoring 時に展開される。
...
`level_menu` 自体に runtime controller はない。`up` / `down` / `left` / `right` / `enter` は展開後の通常の `choice` が処理し、scroll は展開後の通常の container layout が処理する。`level_menu` は inline source や `->` effect を取らない。表示する level の絞り込みは scene の `resources { levels ... }` で指定する。

level iterable の公開 record field は `id`、`name`、`puzzle`、`pack`、`ordinal`、`progress.cleared`。静的 metadata は language/model が供給し、`progress.cleared` は play session が同じ level identity に投影する。したがって通常の `for level in levels` の中で `if level.progress.cleared { ... } else { ... }` を使える。`show_solved = true` はこの式を含む通常の `choice` へ lowering され、level menu 専用の runtime 判定を作らない。`locked = ...` は公開 availability field が未定義なのでエラーになる。`wrap = true` は共通 choice navigation policy が未定義なのでエラーになる。旧 `show index`、`columns <n>`、裸の `wrap`、`action <name>` は読まない。

progress save version 2 は level の表示名ではなく同じ `level.id` を保存する。`levels` の各 entry は `id` と `cleared`、`currentLevel` は同じ id を持つ。name-only の version 1 record は別の level を推測して復元しない。
