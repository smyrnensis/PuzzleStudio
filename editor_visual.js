...
function visualDimensionAdapter(dimension) {
  ...
  return {
    ...
    set frames(frames) {
      if (is3d) state.frames = frames;
      else state.animationFrames = frames;
    },
    validColorIndex: (index) => Number.isInteger(index) && index >= 0 && index < state.palette.length,
    render: is3d ? renderVisual3dBuilder : renderVisualBuilder,
    renderControls: is3d ? renderVisual3dControls : renderVisualControls,
    renderPalette: is3d ? renderVisual3dPalette : renderVisualPalette,
    renderColorSurfaces: is3d ? renderVisual3dColorSurfaces : renderVisualColorSurfaces,
    statusElement: is3d ? visual3dActionStatus : visualActionStatus,
    get statusTimer() {
      return is3d ? visual3dActionClearTimer : visualActionClearTimer;
    },
    set statusTimer(timer) {
      if (is3d) visual3dActionClearTimer = timer;
      else visualActionClearTimer = timer;
    },
    updateBoundColor: is3d ? () => {} : updateVisualBoundColorDefinition,
    afterPaletteRemoval: is3d ? () => {} : updateVisualBoundShapeDefinition,
    commitCells: is3d ? () => {} : () => {
      state.solidSource = false;
      updateVisualBoundShapeDefinition();
    },
    extentKeys: is3d ? ["width", "height", "depth"] : ["width", "height"],
    scaleInput: is3d ? visual3dScaleInput : visualScaleInput,
    afterScale: (direction, factor) => {
      if (!is3d) {
        updateVisualBoundShapeDefinition();
        return;
      }
      resetVisual3dClipState();
      state.slice = Math.min(
        direction === "up" ? state.slice * factor : Math.floor(state.slice / factor),
        visual3dAxisSize() - 1,
      );
...
function visualToolStateAdapter(dimension) {
  const is3d = dimension === "visual3d" || dimension === "3d";
  return {
    dimension: is3d ? "visual3d" : "visual",
    board: is3d ? visual3dSliceBoard : visualBoard,
    get bucketActive() {
      return is3d ? visual3dBucketActive : visualBucketActive;
    },
    set bucketActive(active) {
      if (is3d) visual3dBucketActive = active;
      else visualBucketActive = active;
    },
    get translateActive() {
      return is3d ? visual3dTranslateActive : visualTranslateActive;
    },
    set translateActive(active) {
      if (is3d) visual3dTranslateActive = active;
      else visualTranslateActive = active;
    },
    get translateDrag() {
      return is3d ? visual3dTranslateDrag : visualTranslateDrag;
    },
    set translateDrag(drag) {
      if (is3d) visual3dTranslateDrag = drag;
      else visualTranslateDrag = drag;
    },
    get gridVisible() {
      return is3d ? visual3dGridVisible : visualGridVisible;
    },
    set gridVisible(visible) {
      if (is3d) visual3dGridVisible = visible;
      else visualGridVisible = visible;
    },
    get clipActive() {
      return is3d ? visual3dClipActive : visualClipActive;
    },
    set clipActive(active) {
      if (is3d) visual3dClipActive = active;
      else visualClipActive = active;
    },
    get clipSelection() {
      return is3d ? visual3dClipSelection : visualClipSelection;
    },
    set clipSelection(selection) {
      if (is3d) visual3dClipSelection = selection;
      else visualClipSelection = selection;
    },
    get clipDrag() {
      return is3d ? visual3dClipDrag : visualClipDrag;
    },
    set clipDrag(drag) {
      if (is3d) visual3dClipDrag = drag;
      else visualClipDrag = drag;
    },
    get clipFloating() {
      return is3d ? visual3dClipFloating : visualClipFloating;
    },
    set clipFloating(floating) {
      if (is3d) visual3dClipFloating = floating;
      else visualClipFloating = floating;
    },
    normalizeClipSelection: is3d ? normalizeVisual3dClipBox : normalizeVisualClipRect,
    deactivateClip: is3d
      ? deactivateVisual3dClipMode
      : deactivateVisualClipMode,
    render: is3d ? renderVisual3dBuilder : renderVisualBuilder,
    syncBucket: is3d ? syncVisual3dBucketButton : syncVisualPaintToolControls,
    idleStatus: () => is3d
    ...
    bucketStatus: () => {
      if (!is3d) {
        return visualClipActive
          ? "Bucket: click a connected area inside the clip region"
          : "Bucket: click a connected area";
      }
      const scope = visual3dEditScope();
      if (visual3dClipActive) {
        return scope === "all"
          ? "Bucket: click inside the clip region to fill its 3D component"
          : "Bucket: click inside the clip region to fill its slice component";
      }
      return scope === "all"
        ? "Bucket: click a voxel to fill its 3D component"
        : "Bucket: click a slice area to fill its component";
    },
    translateStatus: () => is3d && visual3dEditScope() === "all"
      ? "Translate: drag the whole visual"
      : is3d ? "Translate: drag the current slice" : "Translate: drag the visual",
    clipSelectStatus: is3d ? "Clip: drag to select an area" : "Clip: drag to select visual area",
  };
}

function toggleVisualEditorClipMode(dimension) {
  const tool = visualToolStateAdapter(dimension);
  if (tool.clipActive) {
    deactivateVisualEditorClipMode(tool.dimension);
    setVisualDimensionActionStatus(tool.dimension, tool.idleStatus(), "is-ok");
    return;
  }
  tool.bucketActive = false;
  tool.translateActive = false;
  tool.clipActive = true;
  tool.clipSelection = tool.normalizeClipSelection(tool.clipSelection);
  tool.clipDrag = null;
  tool.render();
  setVisualDimensionActionStatus(
    tool.dimension,
    tool.clipSelection ? "Clip: drag selection to move it" : tool.clipSelectStatus,
    "is-ok",
  );
}

function deactivateVisualEditorClipMode(dimension, options = {}) {
  const tool = visualToolStateAdapter(dimension);
  const wasActive = tool.clipActive || tool.clipSelection || tool.clipDrag || tool.clipFloating;
  tool.clipActive = false;
  tool.clipSelection = options.clearSelection === false
    ? tool.normalizeClipSelection(tool.clipSelection)
    : null;
  tool.clipDrag = null;
  tool.clipFloating = null;
  if (options.render === false || !wasActive) return;
  tool.render();
}

function syncVisualEditorGridButton(dimension) {
  if (!visualGridButton) return;
  const tool = visualToolStateAdapter(dimension);
  visualGridButton.classList.toggle("is-active", tool.gridVisible);
  visualGridButton.setAttribute("aria-pressed", String(tool.gridVisible));
  visualGridButton.title = "Toggle grid";
  visualGridButton.setAttribute(
    "aria-label",
    tool.dimension === "visual3d" ? "Toggle 3D visual slice grid" : "Toggle visual grid",
  );
}

function toggleVisualEditorGrid(dimension) {
  const tool = visualToolStateAdapter(dimension);
  tool.gridVisible = !tool.gridVisible;
  syncVisualEditorGridButton(tool.dimension);
  if (tool.dimension === "visual3d") {
    renderVisual3dSliceBoard();
    renderVisual3dPresentationSurfaces();
  } else {
    visualBoard?.classList.toggle("is-grid-hidden", !tool.gridVisible);
  }
  const label = tool.dimension === "visual3d" ? "3D visual grid" : "Visual grid";
  setVisualDimensionActionStatus(
    tool.dimension,
    `${label} ${tool.gridVisible ? "visible" : "hidden"}`,
    "is-ok",
  );
}

function toggleVisualEditorBucketMode(dimension) {
  const tool = visualToolStateAdapter(dimension);
  if (!tool.bucketActive) tool.translateActive = false;
  tool.bucketActive = !tool.bucketActive;
  tool.syncBucket();
  if (tool.dimension === "visual") renderVisualPalette();
  setVisualDimensionActionStatus(
    tool.dimension,
    tool.bucketActive ? tool.bucketStatus() : tool.idleStatus(),
    "is-ok",
  );
}

function deactivateVisualEditorBucketModeAfterUse(dimension) {
  const tool = visualToolStateAdapter(dimension);
  if (!tool.bucketActive) return;
  tool.bucketActive = false;
  tool.syncBucket();
}

function toggleVisualEditorTranslateMode(dimension) {
  const tool = visualToolStateAdapter(dimension);
  if (tool.translateActive) {
    deactivateVisualEditorTranslateMode(tool.dimension);
    return;
  }
  tool.bucketActive = false;
  tool.deactivateClip({ render: false });
  tool.translateActive = true;
  tool.translateDrag = null;
  tool.render();
  setVisualDimensionActionStatus(tool.dimension, tool.translateStatus(), "is-ok");
}

function deactivateVisualEditorTranslateMode(dimension, options = {}) {
  const tool = visualToolStateAdapter(dimension);
  const drag = tool.translateDrag;
  const wasActive = tool.translateActive || drag;
  if (drag && tool.board.hasPointerCapture?.(drag.pointerId)) {
    tool.board.releasePointerCapture(drag.pointerId);
  }
  tool.translateActive = false;
  tool.translateDrag = null;
  if (options.render === false || !wasActive) return;
...
function normalizedVisualEditorAnimationFrameCount(value) {
  const parsed = Math.trunc(Number(value));
  const count = Number.isFinite(parsed) ? parsed : 1;
  return Math.max(1, Math.min(VISUAL_ANIMATION_MAX_FRAMES, count));
}

function normalizeVisualEditorFrameCells(dimension, cells) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  const length = state.width * state.height * (adapter.sourceDimension === "3d" ? state.depth : 1);
  return Array.from({ length }, (_, index) => {
    const colorIndex = Array.isArray(cells) ? cells[index] : null;
    return adapter.validColorIndex(colorIndex) ? colorIndex : null;
  });
}

function ensureVisualEditorAnimationState(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  const frames = Array.isArray(adapter.frames) && adapter.frames.length
    ? adapter.frames
    : [normalizeVisualEditorFrameCells(adapter.dimension, state.cells)];
  const requestedFrameCount = Number(state.animationFrameCount);
  state.animationFrameCount = normalizedVisualEditorAnimationFrameCount(
    Number.isFinite(requestedFrameCount) && requestedFrameCount > 0 ? requestedFrameCount : frames.length,
  );
  while (frames.length < state.animationFrameCount) {
    frames.push(normalizeVisualEditorFrameCells(adapter.dimension, frames[frames.length - 1] || state.cells));
  }
  frames.length = state.animationFrameCount;
  adapter.frames = frames.map((frame) => normalizeVisualEditorFrameCells(adapter.dimension, frame));
  state.animationFrameIndex = Math.max(
    0,
    Math.min(state.animationFrameCount - 1, Math.trunc(Number(state.animationFrameIndex) || 0)),
  );
  state.animationPlaybackIndex = Math.max(
    0,
    Math.min(state.animationFrameCount - 1, Math.trunc(Number(state.animationPlaybackIndex) || 0)),
  );
  state.animationDurationMs = normalizedVisualEditorAnimationDuration(state.animationDurationMs);
  if (state.animationMode) state.cells = adapter.frames[state.animationFrameIndex];
}
...
function visualEditorObjectName(input, fallback) {
...
function validVisualPaletteColorIndex(dimension, index) {
  return visualDimensionAdapter(dimension).validColorIndex(index);
}

function normalizedVisualPaletteCellColorIndex(dimension, index) {
  const adapter = visualDimensionAdapter(dimension);
  const colorIndex = adapter.state.cells[index];
  return adapter.validColorIndex(colorIndex) ? colorIndex : null;
}

function visualPaletteColorForIndex(dimension, index) {
  const adapter = visualDimensionAdapter(dimension);
  return adapter.validColorIndex(index)
    ? normalizeVisualColor(adapter.palette[index].color)
    : "#00000000";
}

function visualPaletteInkForIndex(dimension, index) {
  const adapter = visualDimensionAdapter(dimension);
  return adapter.validColorIndex(index)
    ? readableInkForColor(adapter.palette[index].color)
    : "#8d969f";
}

function pasteVisualEditorCell(dimension, index, clipboardValue) {
  const adapter = visualDimensionAdapter(dimension);
  if (clipboardValue === null) return false;
  if (!adapter.validColorIndex(clipboardValue)) {
    throw new Error(`Invalid ${adapter.noun} clip palette index ${clipboardValue}`);
  }
  if (!Number.isInteger(index) || index < 0 || index >= adapter.state.cells.length) return false;
  if (adapter.state.cells[index] === clipboardValue) return false;
  adapter.state.cells[index] = clipboardValue;
  return true;
}

function remapVisualEditorClipboardPalette(dimension, clipboard) {
  if (!Array.isArray(clipboard?.colors)) return clipboard;
  const adapter = visualDimensionAdapter(dimension);
  const colorToIndex = new Map(
    adapter.palette.map((entry, index) => [normalizeVisualColor(entry.color), index]),
  );
  const sourceToTarget = clipboard.colors.map((rawColor) => {
    ...
    if (!colorToIndex.has(color)) {
      if (adapter.palette.length >= VISUAL_COLOR_TOKENS.length) {
        throw new Error(`Paste needs more colors than the ${adapter.noun} palette can hold`);
      }
      colorToIndex.set(color, adapter.palette.length);
      adapter.palette.push({ color });
    }
    return colorToIndex.get(color);
  });
  return {
    ...clipboard,
    cells: clipboard.cells.map((value) => value === null ? null : sourceToTarget[value]),
  };
}

function commitVisualEditorCellMutation(dimension, before, changed, message, emptyMessage) {
  const adapter = visualDimensionAdapter(dimension);
  if (!changed) {
    setVisualDimensionActionStatus(adapter.dimension, emptyMessage, "is-ok");
    adapter.render();
    return false;
  }
  adapter.commitCells();
  adapter.render();
  syncVisualEditorSourceActionButtons(
    adapter.dimension,
    adapter.dimension === "visual" ? { allowActive: true } : {},
  );
  setVisualDimensionActionStatus(adapter.dimension, message, "is-ok");
  setStatus(message, "is-ok");
  pushVisualEditUndoSnapshot(adapter.dimension, before);
  return true;
}

function syncVisualPaletteSwatchesForDimension(dimension, target) {
  const adapter = visualDimensionAdapter(dimension);
  for (const [index, entry] of adapter.palette.entries()) {
    const color = normalizeVisualColor(entry.color);
    const displayName = visualPaletteEntryDisplayName(entry);
    for (const token of target.querySelectorAll(`[data-color-index="${index}"]`)) {
      ...
      token.setAttribute(
        "aria-label",
        displayName ? `Paint color ${index}: ${displayName}` : `Paint color ${index}`,
      );
    }
  }
  const selected = adapter.palette[adapter.state.selectedColorIndex];
  const currentButton = target.querySelector(".visual-current-color-button");
  if (!currentButton || !selected) return;
  ...
  currentButton.setAttribute(
    "aria-label",
    displayName ? `Edit selected color ${displayName}` : `Edit selected color ${normalized}`,
  );
  const currentHexInput = target.querySelector(".visual-current-hex-input");
  if (currentHexInput && !currentHexInput.classList.contains("is-name-mode")
    && document.activeElement !== currentHexInput) {
    currentHexInput.value = normalized;
  }
}

function syncVisualColorAdjustersForDimension(dimension, target) {
  const adapter = visualDimensionAdapter(dimension);
  const selected = adapter.validColorIndex(adapter.state.selectedColorIndex)
    ? adapter.palette[adapter.state.selectedColorIndex]
    : null;
  if (!selected) return;
  const normalized = normalizeVisualColor(selected.color);
  for (const adjuster of target.querySelectorAll(".visual-color-adjuster")) {
    if (!adjuster.contains(document.activeElement)) adjuster.syncColor?.(normalized);
  }
}

function remapVisualEditorFrames(dimension, nextExtent, sourceCoordinates) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  if (adapter.sourceDimension === "3d") commitVisual3dActiveFrame();
  const previous = {
    width: state.width,
    height: state.height,
    depth: adapter.sourceDimension === "3d" ? state.depth : 1,
  };
  const next = {
    width: nextExtent.width,
    height: nextExtent.height,
    depth: adapter.sourceDimension === "3d" ? nextExtent.depth : 1,
  };
  const remap = (frame) => {
    const cells = Array.from({ length: next.width * next.height * next.depth }, () => null);
    for (let z = 0; z < next.depth; z += 1) {
      for (let y = 0; y < next.height; y += 1) {
        for (let x = 0; x < next.width; x += 1) {
          const source = sourceCoordinates(x, y, z, previous, next);
          if (!source || source.x < 0 || source.x >= previous.width
            || source.y < 0 || source.y >= previous.height
            || source.z < 0 || source.z >= previous.depth) continue;
          const sourceIndex = ((source.z * previous.height + source.y) * previous.width) + source.x;
          const targetIndex = ((z * next.height + y) * next.width) + x;
          const colorIndex = frame[sourceIndex];
          cells[targetIndex] = adapter.validColorIndex(colorIndex) ? colorIndex : null;
        }
      }
    }
    return cells;
  };
  const sourceFrames = state.animationMode && adapter.frames.length ? adapter.frames : [state.cells];
  adapter.frames = sourceFrames.map(remap);
  state.width = next.width;
  state.height = next.height;
  if (adapter.sourceDimension === "3d") state.depth = next.depth;
  state.animationFrameCount = adapter.frames.length;
  state.animationFrameIndex = Math.min(state.animationFrameIndex, adapter.frames.length - 1);
  state.animationPlaybackIndex = Math.min(state.animationPlaybackIndex, adapter.frames.length - 1);
  state.cells = adapter.frames[state.animationFrameIndex];
}

function visualEditorScaleFactorForDimension(dimension) {
  return visualEditorScaleFactor(visualDimensionAdapter(dimension).scaleInput, VISUAL_EDITOR_MAX_SIZE);
}

function canScaleDownVisualEditor(dimension, factor = visualEditorScaleFactorForDimension(dimension)) {
  const adapter = visualDimensionAdapter(dimension);
  return factor > 1 && adapter.extentKeys.every((key) => (
    adapter.state[key] >= factor && adapter.state[key] % factor === 0
  ));
}

function scaleVisualEditor(dimension, direction) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  const before = visualEditSnapshot(adapter.dimension);
  const factor = visualEditorScaleFactorForDimension(adapter.dimension);
  if (direction === "down" && !canScaleDownVisualEditor(adapter.dimension, factor)) {
    const extent = adapter.extentKeys.map((key) => state[key]).join("x");
    setVisualDimensionActionStatus(
      adapter.dimension,
      `Dimensions ${extent} are not divisible by ${factor}`,
      "is-error",
    );
    adapter.renderControls();
    return false;
  }
  const next = Object.fromEntries(adapter.extentKeys.map((key) => [
    key,
    direction === "up" ? state[key] * factor : state[key] / factor,
  ]));
  if (direction === "up" && Math.max(...Object.values(next)) > VISUAL_EDITOR_MAX_SIZE) {
    setVisualDimensionActionStatus(
      adapter.dimension,
      `${adapter.noun[0].toUpperCase()}${adapter.noun.slice(1)} size limit is ${VISUAL_EDITOR_MAX_SIZE}`,
      "is-error",
    );
    adapter.renderControls();
    return false;
  }
  const sourceCoordinates = direction === "up"
    ? (x, y, z) => ({ x: Math.floor(x / factor), y: Math.floor(y / factor), z: Math.floor(z / factor) })
    : (x, y, z) => ({ x: x * factor, y: y * factor, z: z * factor });
  remapVisualEditorFrames(adapter.dimension, next, sourceCoordinates);
  adapter.afterScale(direction, factor);
  adapter.render();
  const extent = adapter.extentKeys.map((key) => next[key]).join("x");
  const message = `${direction === "up" ? "Scaled" : "Scaled down"} ${factor}x to ${extent}`;
  setVisualDimensionActionStatus(adapter.dimension, message, "is-ok");
  setStatus(`${direction === "up" ? "Scaled" : "Scaled down"} ${adapter.noun} ${factor}x to ${extent}`, "is-ok");
  pushVisualEditUndoSnapshot(adapter.dimension, before);
  return true;
}

function setVisualDimensionActionStatus(dimension, text, className = "") {
  const adapter = visualDimensionAdapter(dimension);
  const target = adapter.statusElement;
  if (!target) return;
  window.clearTimeout(adapter.statusTimer);
  target.className = `visual-action-status tool-feedback-bar ${className}`.trim();
  target.textContent = text;
  setPaneStatus("visual", text, className);
  if (text && className === "is-ok") {
    adapter.statusTimer = window.setTimeout(() => {
      if (target.textContent === text && target.classList.contains("is-ok")) {
        target.className = "visual-action-status tool-feedback-bar";
        target.textContent = "";
      }
    }, 1800);
  }
...
function clearVisualDimensionActionError(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  if (adapter.statusElement?.classList.contains("is-error")) {
    setVisualDimensionActionStatus(dimension, "");
  }
}
...
function resetVisualEditorBuilder(dimension, extent, options = {}) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  options.beforeReset?.();
  for (const key of adapter.extentKeys) state[key] = clampVisualEditorSize(extent[key]);
  options.configureState?.(state);
  const cellCount = state.width * state.height * (adapter.sourceDimension === "3d" ? state.depth : 1);
  state.cells = Array.from({ length: cellCount }, () => null);
  adapter.frames = [state.cells.slice()];
  state.animationFrameIndex = 0;
  state.animationFrameCount = 1;
  state.animationPlaybackIndex = 0;
  state.shapeBind = null;
  state.sourcePreludeRows = [];
  state.sourceSpatialOps = [];
  if (!adapter.validColorIndex(state.selectedColorIndex)) state.selectedColorIndex = 0;
  adapter.render();
}

function resetVisualBuilder(width = visual.width, height = visual.height) {
  resetVisualEditorBuilder("visual", { width, height }, {
...
function normalizedVisualAnimationFrameCount(value = visual.animationFrameCount) {
  return normalizedVisualEditorAnimationFrameCount(value);
}
...
function ensureVisualAnimationFrames() {
  ensureVisualEditorAnimationState("visual");
}

function normalizeVisualAnimationFrameCells(cells) {
  return normalizeVisualEditorFrameCells("visual", cells);
}
...
function sharedVisualAnimationController(dimension = currentVisualPaneMode) {
  ...
  return {
    ...
    maxFrames: VISUAL_ANIMATION_MAX_FRAMES,
...
function selectSharedVisualAnimationFrame(dimension, index) {
  ...
  context.render();
  setVisualDimensionActionStatus(context.dimension, `Frame ${nextIndex + 1}`, "is-ok");
}
...
function insertSharedVisualAnimationFrameAt(dimension, index) {
  ...
  if (context.state.animationFrameCount >= context.maxFrames) {
    context.render();
    setVisualDimensionActionStatus(context.dimension, `Maximum ${context.maxFrames} frames`, "is-error");
    return false;
  }
  ...
  context.deactivateClip();
  context.render();
  setVisualDimensionActionStatus(context.dimension, `Added frame ${insertIndex + 1}`, "is-ok");
...
function removeSharedVisualAnimationFrameAt(dimension, index) {
  ...
  if (context.state.animationFrameCount <= 1) {
    context.render();
    setVisualDimensionActionStatus(context.dimension, "At least 1 frame is required", "is-error");
    return false;
  }
  const before = visualEditSnapshot(context.dimension);
  context.commit();
  ...
  context.deactivateClip();
  context.render();
  setVisualDimensionActionStatus(context.dimension, `Removed frame ${removeIndex + 1}`, "is-ok");
  pushVisualEditUndoSnapshot(context.dimension, before);
  return true;
}
...
function updateVisualAnimationDuration(value, options = {}) {
  return updateVisualEditorAnimationDuration("visual", value, options);
}

function updateVisualEditorAnimationDuration(dimension, value, options = {}) {
  const adapter = visualDimensionAdapter(dimension);
  const input = adapter.dimension === "visual3d"
    ? visual3dAnimationDurationInput
    : visualAnimationDurationInput;
  const nextDuration = normalizedVisualEditorAnimationDuration(value);
  const changed = nextDuration !== adapter.state.animationDurationMs;
  const before = options.recordHistory === false || !changed
    ? null
    : visualEditSnapshot(adapter.dimension);
  adapter.state.animationDurationMs = nextDuration;
  if (
    input
    && !(options.preserveInput === true && document.activeElement === input)
  ) {
    input.value = String(adapter.state.animationDurationMs);
  }
  if (changed && adapter.state.animationMode && adapter.state.animationFrameCount > 1) {
    stopVisualAnimationPlayback({ render: false });
    startVisualAnimationPlayback();
  }
  if (adapter.dimension === "visual3d") renderVisual3dControls();
  if (before) pushVisualEditUndoSnapshot(adapter.dimension, before);
  return changed;
}
...
function visualAnimationFrameDelayMs() {
  ...
  return visualEditorFrameDelayMs(context.durationMs(), context.state.animationFrameCount);
}

function visualEditorFrameDelayMs(durationMs, frameCount) {
  return Math.max(1, Math.round(durationMs / frameCount));
}

let visualAssetExportPending = false;

async function exportVisualAsset(format) {
  const dimension = format === "vox" ? "visual3d" : "visual";
  if (currentVisualPaneMode !== dimension || visualAssetExportPending) return;
  const adapter = visualDimensionAdapter(dimension);
  const state = adapter.state;
  const nameInput = dimension === "visual3d" ? visual3dNameInput : visualNameInput;
  const status = (text, tone) => setVisualDimensionActionStatus(dimension, text, tone);
  visualAssetExportPending = true;
  try {
    // Snapshot the authored draft before asynchronous encoding. The selected
    // cells can contain edits newer than the animation frame array.
    const frames = format !== "png" && state.animationMode
      ? adapter.frames.map((frame, index) =>
        (index === state.animationFrameIndex ? state.cells : frame).slice())
      : [state.cells.slice()];
    const snapshot = {
      width: state.width,
      height: state.height,
      palette: state.palette.map(entry => {
        const color = window.PuzzleStudioColorEditor.parseColor(normalizeVisualColor(entry.color));
        return [color.red, color.green, color.blue, color.alpha];
      }),
      frames,
      ...(format === "vox" ? {depth: state.depth} : {
        frameDelayMs: visualEditorFrameDelayMs(normalizedVisualAnimationDuration(), frames.length),
      }),
    };
    const filename = `${sanitizeFileName(nameInput.value.trim()) || "visual"}.${format}`;
    status(`Exporting ${format.toUpperCase()}…`);
    const blob = await window.PuzzleStudioVisualExport.encode(snapshot, format);
    downloadBlob(blob, filename);
    status(`Exported ${filename}`, "is-ok");
  } catch (error) {
    status(`Could not export visual: ${error?.message || error}`, "is-error");
  } finally {
    visualAssetExportPending = false;
  }
}
...
function syncVisualAnimationPlayback() {
  ...
  if (
    !context.state.animationPlaying
...
function renderSharedVisualAnimationPlaybackView(context, frame) {
  ...
  if (context.dimension === "visual3d") {
    queueVisual3dRuntimePlayback();
    return;
  }
...
function toggleVisualBucketMode() {
  toggleVisualEditorBucketMode("visual");
}

function deactivateVisualBucketModeAfterUse() {
  deactivateVisualEditorBucketModeAfterUse("visual");
}
...
function closeVisualPaletteEditor(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  commitVisualColorEditHistory(adapter.dimension);
  adapter.state.addPaletteOpen = false;
  adapter.state.editPaletteOpen = false;
  adapter.state.customColorOpen = false;
  adapter.state.addDraftColorIndex = null;
  adapter.renderPalette();
}

function toggleVisualPaletteAdd(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  commitVisualColorEditHistory(adapter.dimension);
  const before = visualEditSnapshot(adapter.dimension);
  const opening = !state.addPaletteOpen;
  if (opening && adapter.palette.length >= VISUAL_COLOR_TOKENS.length) {
    setVisualDimensionActionStatus(
      adapter.dimension,
      `Palette limit is ${VISUAL_COLOR_TOKENS.length} colors`,
      "is-error",
    );
    return;
  }
  state.addPaletteOpen = opening;
  state.editPaletteOpen = false;
  state.customColorOpen = opening;
  if (opening && !adapter.validColorIndex(state.addDraftColorIndex)) {
    adapter.palette.push({ color: normalizeVisualColor(nextVisualPresetColor(adapter.palette)) });
    state.addDraftColorIndex = adapter.palette.length - 1;
    state.selectedColorIndex = state.addDraftColorIndex;
  } else if (!opening) {
    state.addDraftColorIndex = null;
  }
  adapter.render();
  pushVisualEditUndoSnapshot(adapter.dimension, before);
}

function previewNewVisualPaletteColor(dimension, color, options = {}) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  const before = options.deferHistory ? null : visualEditSnapshot(adapter.dimension);
  if (options.deferHistory) beginVisualColorEditHistory(adapter.dimension);
  if (!adapter.validColorIndex(state.addDraftColorIndex) && adapter.palette.length >= VISUAL_COLOR_TOKENS.length) {
    return;
  }
  if (!adapter.validColorIndex(state.addDraftColorIndex)) {
    adapter.palette.push({ color: normalizeVisualColor(color) });
    state.addDraftColorIndex = adapter.palette.length - 1;
    state.selectedColorIndex = state.addDraftColorIndex;
    adapter.render();
  } else {
    adapter.palette[state.addDraftColorIndex].color = normalizeVisualColor(color);
    state.selectedColorIndex = state.addDraftColorIndex;
    adapter.renderColorSurfaces();
  }
  if (options.closeMenu) {
    state.addPaletteOpen = false;
    state.editPaletteOpen = false;
    state.customColorOpen = false;
    state.addDraftColorIndex = null;
    adapter.render();
  }
  if (!options.deferHistory) pushVisualEditUndoSnapshot(adapter.dimension, before);
}

function updateSelectedVisualPaletteColor(dimension, value, options = {}) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  const before = options.deferHistory || options.commitHistory
    ? null
    : visualEditSnapshot(adapter.dimension);
  if (options.deferHistory || options.commitHistory) beginVisualColorEditHistory(adapter.dimension);
  if (!adapter.validColorIndex(state.selectedColorIndex)) state.selectedColorIndex = 0;
  const selected = adapter.palette[state.selectedColorIndex];
  if (!selected) return;
  const normalized = normalizeVisualColor(value);
  selected.color = normalized;
  adapter.updateBoundColor(selected, normalized);
  if (options.closeMenu) {
    state.editPaletteOpen = false;
    state.customColorOpen = false;
    state.addDraftColorIndex = null;
  ...
  if (options.commitHistory) commitVisualColorEditHistory(adapter.dimension);
  else pushVisualEditUndoSnapshot(adapter.dimension, before);
}

function removeVisualPaletteColorAt(dimension, deletedIndex) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  if (!adapter.validColorIndex(deletedIndex) || adapter.palette.length <= 1) return false;
  const oldPaletteLength = adapter.palette.length;
  adapter.palette.splice(deletedIndex, 1);
  const remapCell = (colorIndex) => {
    if (!Number.isInteger(colorIndex) || colorIndex < 0 || colorIndex >= oldPaletteLength) return null;
    if (colorIndex === deletedIndex) return null;
    return colorIndex > deletedIndex ? colorIndex - 1 : colorIndex;
  };
  const activeCells = state.cells.map(remapCell);
  const frames = Array.isArray(adapter.frames)
    ? adapter.frames.map((frame) => Array.isArray(frame) ? frame.map(remapCell) : frame)
    : [];
  const activeFrameIndex = state.animationMode ? state.animationFrameIndex : 0;
  while (frames.length <= activeFrameIndex) frames.push(activeCells.slice());
  frames[activeFrameIndex] = activeCells;
  adapter.frames = frames;
  state.cells = frames[activeFrameIndex];
  state.selectedColorIndex = Math.min(deletedIndex, adapter.palette.length - 1);
  return true;
}

function removeSelectedVisualPaletteColor(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  commitVisualColorEditHistory(adapter.dimension);
  const before = visualEditSnapshot(adapter.dimension);
  if (!adapter.validColorIndex(state.selectedColorIndex) || adapter.palette.length <= 1) return;
  state.addPaletteOpen = false;
  state.editPaletteOpen = false;
  state.customColorOpen = false;
  state.addDraftColorIndex = null;
  removeVisualPaletteColorAt(adapter.dimension, state.selectedColorIndex);
  adapter.afterPaletteRemoval();
  adapter.render();
  pushVisualEditUndoSnapshot(adapter.dimension, before);
}

function cancelVisualPaletteColorAdd(dimension) {
  const adapter = visualDimensionAdapter(dimension);
  const { state } = adapter;
  discardVisualColorEditHistory(adapter.dimension);
  const before = visualEditSnapshot(adapter.dimension);
  if (adapter.validColorIndex(state.addDraftColorIndex)) {
    removeVisualPaletteColorAt(adapter.dimension, state.addDraftColorIndex);
  }
  state.addPaletteOpen = false;
  state.editPaletteOpen = false;
  state.customColorOpen = false;
...
function newVisualEditorDraft({ dimension, nameInput, name, reset }) {
  const adapter = visualDimensionAdapter(dimension);
  const before = visualEditSnapshot(adapter.dimension);
  clearVisualEditorDimensionSourceTarget(adapter.dimension);
  nameInput.value = name;
  adapter.state.palette = [{ color: "#ff004d" }];
  adapter.state.selectedColorIndex = 0;
  adapter.state.animationMode = false;
  reset();
  setVisualDimensionActionStatus(adapter.dimension, `Started new ${adapter.noun}`, "is-ok");
  pushVisualEditUndoSnapshot(adapter.dimension, before);
}

function applyIncompleteVisualEditorSourceTarget({
  dimension,
  name,
  target,
  nameInput,
  ...
  if (target && typeof target === "object") {
    setVisualEditorDimensionSourceTarget(adapter.dimension, target, activeDocument());
  }
  nameInput.value = name || "";
  for (const key of adapter.extentKeys) state[key] = clampVisualEditorSize(state[key]);
  if (adapter.dimension === "visual3d") {
    state.axis = "z";
    state.slice = 0;
    state.hoverSlice = null;
  }
  state.palette = [];
  state.shapeBind = null;
  if (adapter.dimension === "visual") state.solidSource = false;
  state.sourcePreludeRows = Array.isArray(target?.sourceVisual?.preludeRows)
    ? target.sourceVisual.preludeRows.slice()
    : [];
  state.sourceSpatialOps = Array.isArray(target?.sourceVisual?.spatialOps)
  ...
  state.animationFrameCount = 1;
  state.animationPlaybackIndex = 0;
  state.animationDurationMs = adapter.dimension === "visual3d" ? null : 120;
  if (adapter.dimension === "visual3d") state.frameDurationMs = null;
  state.cells = Array.from(
    { length: state.width * state.height * (adapter.dimension === "visual3d" ? state.depth : 1) },
    () => null,
  );
  adapter.frames = [state.cells.slice()];
...
function applyLoadedVisualEditorSource({
  dimension,
  name,
  nameInput,
  loaded,
  ...
  beforeApply();
  nameInput.value = name;
  for (const key of adapter.extentKeys) state[key] = loaded[key];
  configureState(state, loaded);
  state.palette = loaded.palette;
  state.shapeBind = loaded.shapeBind || null;
  state.sourcePreludeRows = Array.isArray(loaded.sourcePreludeRows) ? loaded.sourcePreludeRows : [];
  state.sourceSpatialOps = Array.isArray(loaded.sourceSpatialOps) ? loaded.sourceSpatialOps : [];
  state.animationMode = Boolean(animationMode);
  state.animationFrameIndex = 0;
  state.animationPlaybackIndex = 0;
  state.animationPlaying = false;
  adapter.frames = frames.map((frame) => frame.slice());
  state.animationFrameCount = Math.max(1, adapter.frames.length);
  state.cells = adapter.frames[0];
  if (state.animationMode) state.animationDurationMs = loaded.animationDurationMs;
  ensureVisualEditorAnimationState(adapter.dimension);
  if (adapter.dimension === "visual3d") {
    state.animationDurationMs = loaded.animationDurationMs;
    state.frameDurationMs = loaded.frameDurationMs;
  }
  state.selectedColorIndex = state.palette.length ? 0 : null;
  state.addPaletteOpen = false;
  state.editPaletteOpen = false;
  state.customColorOpen = false;
  state.addDraftColorIndex = null;
...
function renderVisualPaletteContentForDimension({
  dimension,
  target,
  shapeField,
  leadingControl,
  ...
  const selectedIsTransparent = state.selectedColorIndex === null;
  if (selectedIsTransparent || adapter.validColorIndex(state.selectedColorIndex)) {
    const selected = selectedIsTransparent
      ? { color: "#00000000" }
      : adapter.palette[state.selectedColorIndex];
    const selectedBind = selectedIsTransparent
      ? { available: false, linked: false, name: "" }
      : visualPaletteEntryBindInfo(selected);
    ...
    currentButton.title = selectedIsTransparent ? "Transparent eraser cannot be edited" : "Edit selected color";
    currentButton.setAttribute(
      "aria-label",
      selectedIsTransparent
        ? "Selected transparent eraser color #00000000, not editable"
        : selectedDisplayName
          ? `Edit selected color ${selectedDisplayName}`
          : `Edit selected color ${selected.color}`,
    );
    currentButton.setAttribute("aria-disabled", String(selectedIsTransparent));
    currentButton.setAttribute("aria-expanded", String(!selectedIsTransparent && state.editPaletteOpen));
    currentButton.innerHTML = `<span class="visual-current-color-swatch" aria-hidden="true"></span>`;
    currentButton.insertAdjacentHTML("beforeend", selectedIsTransparent
      ? `<span class="visual-current-transparent-icon" aria-hidden="true">${editorIconSvg("eraser")}</span>`
      : `<span class="visual-current-edit-icon" aria-hidden="true">${editorIconSvg("pencil")}</span>`);
    ...
    const applyCurrentHex = (options = {}) => {
      if (currentHexInput.classList.contains("is-name-mode")) {
        const before = visualEditSnapshot(adapter.dimension);
        const changed = applyVisualPaletteColorName(
          adapter.dimension,
          state.selectedColorIndex,
          currentHexInput.value,
          { reportError: true },
        );
        if (changed && options.commitHistory) pushVisualEditUndoSnapshot(adapter.dimension, before);
        return;
      }
      const parsed = parseVisualHexColor(currentHexInput.value);
      if (!parsed) {
        if (options.reportError) {
          setVisualDimensionActionStatus(adapter.dimension, "Use #rrggbb or #rrggbbaa", "is-error");
        }
        return;
      }
      updateSelectedVisualPaletteColor(adapter.dimension, parsed, {
    ...
    if (!selectedIsTransparent) {
      currentButton.addEventListener("click", () => {
        const opening = !state.editPaletteOpen;
        if (!opening) commitVisualColorEditHistory(adapter.dimension);
        state.editPaletteOpen = opening;
        state.addPaletteOpen = false;
        state.addDraftColorIndex = null;
        state.customColorOpen = opening;
        if (opening) state.shapeTagPickerOpen = false;
        adapter.renderControls();
        adapter.renderPalette();
      });
      currentHexInput.addEventListener("input", () => {
        if (!currentHexInput.classList.contains("is-name-mode")) applyCurrentHex();
      });
    ...
    if (!selectedIsTransparent) {
      ...
      if (selectedBind.linked && selectedBind.name) {
        currentWrap.append(renderVisualCurrentColorUnlinkButton(
          adapter.dimension,
          state.selectedColorIndex,
          selectedBind,
        ));
      }
      if (state.colorTagPickerOpen) {
        const colorAssets = visualSourceColorAssets(adapter.dimension);
        const picker = renderVisualAssetNamePicker({
          className: "visual-color-tag-picker",
          names: visualColorAssetNames(adapter.dimension),
          value: selectedBind.name || defaultVisualAssetName("color", state.selectedColorIndex, adapter.dimension),
          placeholder: "color_name",
          ariaLabel: "Color tag name",
          emptyText: "No named colors yet",
          optionMeta: (name) => ({ color: colorAssets.get(name) }),
          onCommit: (name) => applyVisualPaletteColorName(adapter.dimension, state.selectedColorIndex, name, { reportError: true }),
          onCancel: () => {
            state.colorTagPickerOpen = false;
            adapter.renderPalette();
          },
        });
        currentWrap.append(picker);
        requestAnimationFrame(() => focusVisualTagPickerInput(picker));
      }
      if (state.editPaletteOpen) {
        ...
        const editMenu = renderVisualColorMenu({
          ...
          onChange: (color, options) => updateSelectedVisualPaletteColor(adapter.dimension, color, options),
          onPreset: (color, options) => updateSelectedVisualPaletteColor(adapter.dimension, color, options),
          renderPalette: adapter.renderPalette,
        });
    ...
    currentWrap.append(shapeField);
    target.append(currentWrap);
    if (pendingEditMenu) positionVisualColorMenu(pendingEditMenu, currentButton, { side: "left" });
  }

  renderVisualPaletteGrid({
    target,
    leadingControl,
    entries: adapter.palette,
    ...
    colorAriaLabel,
    onSelect,
    onAdd: () => toggleVisualPaletteAdd(adapter.dimension),
    onRemove,
    addOpen: state.addPaletteOpen,
    renderAddMenu: () => renderVisualColorMenu({
      mode: "add",
      customValue: adapter.validColorIndex(state.addDraftColorIndex)
        ? adapter.palette[state.addDraftColorIndex].color
        : nextVisualPresetColor(adapter.palette),
      onDiscard: () => cancelVisualPaletteColorAdd(adapter.dimension),
      onChange: (color, options) => previewNewVisualPaletteColor(adapter.dimension, color, options),
      onPreset: (color, options) => previewNewVisualPaletteColor(adapter.dimension, color, options),
      renderPalette: adapter.renderPalette,
    }),
  });
  renderVisualEditorToolbar({ dimension: adapter.sourceDimension, target: adapter.dimension === "visual3d" ? visual3dToolbarHost : visualToolbarHost });
}
...
const VISUAL_EDITOR_TOOL_ORDER = Object.freeze([
  "scope",
  "grid",
  "clip",
  "fill",
  ...
  "flip-horizontal",
  "flip-vertical",
  ...VISUAL_EDIT_COMMANDS.map(({ id }) => id),
]);
...
function renderVisualEditorToolbar({ dimension, target }) {
  ...
  for (const key of VISUAL_EDITOR_TOOL_ORDER) {
...
function renderVisualCurrentColorUnlinkButton(dimension, index, bind) {
  const adapter = visualDimensionAdapter(dimension);
  const button = document.createElement("button");
  button.type = "button";
  ...
  button.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    adapter.state.colorTagPickerOpen = false;
    adapter.state.editPaletteOpen = false;
    applyVisualPaletteColorName(adapter.dimension, index, bind.name, { linked: false });
  });
  return button;
}
...
function visualColorAssetNames(dimension = "visual") {
  return [...visualSourceColorAssets(dimension).keys()].sort((a, b) => a.localeCompare(b));
}

function visualShapeAssetNames(dimension = "visual") {
  return [...visualSourceShapeAssets(dimension).keys()].sort((a, b) => a.localeCompare(b));
}

function activeVisualSourceContract(dimension = "visual") {
  const state = visualDimensionAdapter(dimension).state;
  return state.sourceVisualContract && typeof state.sourceVisualContract === "object"
    ? state.sourceVisualContract
    : null;
}

function visualSourceColorAssets(dimension = "visual") {
  const assets = new Map();
  const contract = activeVisualSourceContract(dimension);
...
function visualSourceShapeAssets(dimension = "visual") {
  const assets = new Map();
  const contract = activeVisualSourceContract(dimension);
...
function applyVisualPaletteColorName(dimension, index, rawName, options = {}) {
  const adapter = visualDimensionAdapter(dimension);
  if (!adapter.validColorIndex(index)) return false;
  const entry = adapter.palette[index];
  const name = sanitizeVisualColorAssetRef(rawName);
  if (!name) {
    if (options.reportError) {
      setVisualDimensionActionStatus(adapter.dimension, "Enter a color name", "is-error");
    }
    return false;
  }
  if (options.linked === false) {
    entry.bind = { type: "color", name, linked: false };
    adapter.state.colorTagPickerOpen = false;
    adapter.renderPalette();
    return true;
  }
  ...
  if (colorAssets.has(name)) {
    ...
    if (!resolved) {
      if (options.reportError) {
        setVisualDimensionActionStatus(adapter.dimension, `Cannot resolve color ${name}`, "is-error");
      }
      return false;
    }
    ...
    const staged = adapter.palette.find((candidate, candidateIndex) => {
  ...
  entry.editMode = "name";
  syncVisualPaletteEntriesForColorName(name, entry.color, adapter.dimension);
  adapter.state.colorTagPickerOpen = false;
  setVisualDimensionActionStatus(adapter.dimension, status, "is-ok");
  adapter.render();
  return true;
}

function applyCurrentColorName(index, rawName, options = {}) {
  return applyVisualPaletteColorName("visual", index, rawName, options);
}
...
function defaultVisualAssetName(kind, index = 0, dimension = "visual") {
  const objectName = dimension === "visual3d" ? visual3dObjectName() : visualObjectName();
  if (kind === "color") {
    const base = sanitizeVisualAssetName(String(objectName).split(":")[0]) || "visual";
    return `${base}_${Number(index) + 1}`;
  }
  const base = sanitizeVisualAssetName(objectName).replace(new RegExp(`_${kind}$`), "") || "visual";
  return `${base}_${kind}_${Number(index) + 1}`;
}

function syncVisualPaletteEntriesForColorName(name, color, dimension = "visual") {
  const normalized = normalizeVisualColor(color);
  for (const entry of visualDimensionAdapter(dimension).palette) {
    const bind = visualPaletteEntryBindInfo(entry);
...
function activeVisualEditDocument() {
  return activeVisualEditorDocument("visual", { allowActive: true }) || activePreviewDocument();
}
...
function setVisualEditSource(entry, document = activeDocument()) {
  ...
  setVisualEditorDimensionSourceTarget("visual", normalized, document);
}

function clearVisualEditSource() {
  clearVisualEditorDimensionSourceTarget("visual");
}

function invalidateVisualEditSourceForDocument(document = activeDocument()) {
  return invalidateVisualEditorDimensionSourceTarget("visual", document);
}

function activeVisualEditSource() {
  return activeVisualEditorSource("visual", { allowActive: true });
}
...
function toggleVisualTranslateMode() {
  toggleVisualEditorTranslateMode("visual");
}

function deactivateVisualTranslateMode(options = {}) {
  deactivateVisualEditorTranslateMode("visual", options);
}
...
function toggleVisualClipMode() {
  toggleVisualEditorClipMode("visual");
}

function deactivateVisualClipMode(options = {}) {
  deactivateVisualEditorClipMode("visual", options);
}
...
function pasteVisualClipCell(index, clipboardValue) {
  return pasteVisualEditorCell("visual", index, clipboardValue);
}

function visualClipCellsForCurrentPalette(clipboard) {
  return remapVisualEditorClipboardPalette("visual", clipboard)?.cells;
}
...
function syncVisualGridButton() {
  syncVisualEditorGridButton("visual");
}

function toggleVisualGrid() {
  toggleVisualEditorGrid("visual");
}
...
function updateSelectedVisualColor(value, options = {}) {
  updateSelectedVisualPaletteColor("visual", value, options);
}

function toggleVisualAddPalette() {
  toggleVisualPaletteAdd("visual");
}
...
function previewNewVisualColor(color, options = {}) {
  previewNewVisualPaletteColor("visual", color, options);
}
...
function cancelVisualColorAdd() {
  cancelVisualPaletteColorAdd("visual");
}
...
function deleteSelectedVisualColor() {
  removeSelectedVisualPaletteColor("visual");
}

function removeVisualPaletteColor(deletedIndex) {
  return removeVisualPaletteColorAt("visual", deletedIndex);
}
...
function syncVisualPaletteSwatches() {
  syncVisualPaletteSwatchesForDimension("visual", visualPalette);
}

function syncVisualColorAdjusters() {
  syncVisualColorAdjustersForDimension("visual", visualPalette);
}

function validVisualColorIndex(index) {
  return validVisualPaletteColorIndex("visual", index);
}
...
function visualColorForColorIndex(index) {
  return visualPaletteColorForIndex("visual", index);
}

function visualInkForColorIndex(index) {
  return visualPaletteInkForIndex("visual", index);
}
...
function updateVisualDimension(axis, value, options = {}) {
  const before = visualEditSnapshot("visual");
  ...
  if (next.width === visual.width && next.height === visual.height) {
    if (!options.preserveInput) renderVisualControls();
    return;
  }
...
function remapVisualFrames(next, sourceCoordinates) {
  remapVisualEditorFrames("visual", { ...next, depth: 1 }, (x, y, _z, previous, extent) => {
    const source = sourceCoordinates(x, y, previous, extent);
    return source ? { ...source, z: 0 } : null;
  });
}

function visualScaleFactor() {
  return visualEditorScaleFactorForDimension("visual");
}

function canScaleDownVisual(factor = visualScaleFactor()) {
  return canScaleDownVisualEditor("visual", factor);
}

function scaleUpVisual() {
  return scaleVisualEditor("visual", "up");
}

function scaleDownVisual() {
  return scaleVisualEditor("visual", "down");
}
...
function normalizedVisualCellColorIndex(index) {
  return normalizedVisualPaletteCellColorIndex("visual", index);
}
...
function startVisualPaint(event) {
  ...
  if (!erase && visualTranslateActive) {
    event.preventDefault();
  ...
  visualPaintDrag = {
    pointerId: event.pointerId,
    colorIndex: erase ? null : visual.selectedColorIndex,
    lastPoint: null,
    geometry,
    beforeSnapshot: visualEditSnapshot("visual"),
    changed: false,
...
function visualObjectName() {
  return visualEditorObjectName(visualNameInput, "Visual");
}
...
function renderVisualShapeBindControl(target, options) {
  ...
  const dimension = state === visual3d ? "visual3d" : "visual";
  ...
  if (state.shapeTagPickerOpen) {
    const picker = renderVisualAssetNamePicker({
      className: "visual-shape-tag-picker",
      names: visualShapeAssetNames(dimension),
      value: info.name || "",
      placeholder: "",
      ariaLabel: "Shape tag name",
      emptyText: "No named shapes yet",
...
function loadVisualSourceTarget(target, options = {}) {
  ...
  applyLoadedVisualEditorSource({
    dimension: "visual",
    name: targetName || "Visual",
    nameInput: visualNameInput,
    loaded,
    frames: loaded.animationMode && Array.isArray(loaded.animationFrames)
      ? loaded.animationFrames
      : [loaded.cells],
    animationMode: loaded.animationMode,
    configureState: (state) => {
      state.solidSource = Boolean(loaded.solid);
    },
  });
  if (!options.silent) {
...
function setVisualActionStatus(text, className = "") {
  setVisualDimensionActionStatus("visual", text, className);
}

function clearVisualActionError() {
  clearVisualDimensionActionError("visual");
}
...
function visualSourceCursorPosition(source, document = activeDocument()) {
  if (document?.id === activeDocument()?.id && sourceEditor) {
    return Math.max(
      0,
      Math.min(String(source || "").length, Math.max(sourceEditor.selection().from || 0, sourceEditor.selection().to || 0)),
    );
  }
...
function canReplaceCurrentVisualDefinition(source) {
  return canReplaceVisualEditorDefinition("visual", source);
}

function syncVisualSourceActionButtons() {
  syncVisualEditorSourceActionButtons("visual", { allowActive: true });
}

function currentVisualEditSourceRange(source) {
  return currentVisualEditorSourceRange("visual", source);
}
...
function bindVisualEditorDimensionInput(input, axis, update) {
  input?.addEventListener("input", () => {
    if (input.validity.valid && input.value !== "") {
      update(axis, input.value, { preserveInput: true });
    }
  });
  input?.addEventListener("change", () => update(axis, input.value));
  input?.addEventListener("keydown", (event) => {
    if (event.key !== "Enter") return;
    event.preventDefault();
    update(axis, input.value);
  });
}
function bindVisualDimensionInput(input, axis) {
  bindVisualEditorDimensionInput(input, axis, updateVisualDimension);
}
...
visualBoard.addEventListener("contextmenu", (event) => event.preventDefault());
