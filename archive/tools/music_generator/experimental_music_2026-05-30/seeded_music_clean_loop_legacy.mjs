...
export function generateSong(seed, options = {}) {
  ...
  const tone = clamp(Number(options.tone ?? options.height ?? 0.62), 0, 1);
...
export function createPlayer(audioContext, playbackScore) {
  ...
  const scheduleAheadSeconds = Math.min(loopSeconds + 0.5, 90);
...
function normalizeBars(value) {
  const bars = Number(value ?? DEFAULT_BARS);
  return BAR_OPTIONS.includes(bars) ? bars : DEFAULT_BARS;
}
