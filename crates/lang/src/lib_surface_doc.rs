...
impl ParseSnapshot {
    ...
    fn from_scan(
        ...
        if let Some(Err(report)) = &parser_catalog {
            document
                .diagnostics
...
fn validate_parser_recognition_completeness(
    ...
    let missing = document
        ...
        .find(|fact| {
            matches!(fact.kind, source::lexer::SourceLexicalKind::Word)
                && !document
                    .semantic_tokens
                    .iter()
                ...
                && !document
                    ...
                    .raw_ranges
                    .iter()
                    .chain(&document.highlight_ranges.plain_ranges)
