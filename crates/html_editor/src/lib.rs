            ...
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    return Err(AppError::Config(
                        "usage: cargo run -p html-editor -- [path/to/game-folder-or-game.puzzle] [-o editor.html] [--serve] [--port 8787]"
                            .to_string(),
                    ));
                }
...
fn source_json(state: &EditorState) -> String {
    let source = fs::read_to_string(&state.puzzle_path).unwrap_or_else(|_| state.source.clone());
    let mut out = String::new();
    out.push('{');
