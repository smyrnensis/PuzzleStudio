...
let componentEmbedActive = () => componentEmbedMode;
...
function resolveRenderMoment(renderScene, moment) {
  if (standaloneRuntime) {
    return standaloneRuntime.resolveRenderMoment(renderScene, moment);
  }
  return requestJson("/api/render-moment", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ renderScene, moment }),
  });
}

function render(state) {
  currentState = state;
  pendingPresentationContinuation = presentationContinuationToken(
    state?.presentationContinuation,
  );
...
function presentationContinuationToken(value) {
  ...
  if (
    ...
    || value.waits.some((wait) => (
      !wait
      || typeof wait !== "object"
      || Array.isArray(wait)
      || typeof wait.model !== "string"
      || !wait.model
      || !Number.isSafeInteger(wait.sequence)
      || wait.sequence < 1
    ))
  ) {
    throw new Error("Runtime returned an invalid presentation continuation token.");
  }
  return value;
}
...
function clampScreenScaleToFrame() {
  if (componentEmbedActive() || !screenFrame || !screenView) {
    return;
  }
...
function focusedViewportModel(state) {
  const focusedModels = new Set(focusedViewportSources(state)
    .map((source) => source?.id?.model)
    .filter((model) => typeof model === "string" && model));
  return focusedModels.size === 1 ? [...focusedModels][0] : "";
}

function focusedViewportSources(state) {
  const focus = state?.surface?.focus;
  return (state?.viewportSources || [])
    .filter((source) => source?.id?.component === focus);
}

function focusedEditorPreviewViewport(state) {
  const sources = focusedViewportSources(state);
...
function notifyPreviewState(state) {
  ...
  const viewport = focusedEditorPreviewViewport(state);
  window.parent.postMessage({
    type: "PuzzleStudioPreviewState",
    levelIndex: state.levelIndex,
    inputs: state.inputs,
    activeModel: focusedViewportModel(state),
    ...
    rawScene: viewport?.state || null,
    scene: viewport?.state?.renderScene || null,
    theme: state.theme,
  }, "*");
}
...
function renderSurface(state) {
  ...
  /* puzzle-host:optional:puzzle3:start */
  if (puzzle3PreviewSurface && renderEmbeddedPuzzleComponent([])) {
    return;
  }
  /* puzzle-host:optional:puzzle3:end */
...
function runtimeViewportSourceId(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)
    ...
    throw new Error("Resolved viewport is missing its typed model/component/source identity");
  }
  return value;
}
...
function runtimeViewportSourceState(sourceId, state = currentState) {
  ...
  const entry = state.viewportSources.find((candidate) =>
    candidate?.id?.model === id.model
      && candidate?.id?.component === id.component
      && candidate?.id?.source === id.source
  );
  if (!entry || !entry.state || typeof entry.state !== "object" || Array.isArray(entry.state)) {
    throw new Error(
      `Viewport source is missing from the runtime registry: ${id.model}/${id.component}/${id.source}`,
    );
  }
  return entry.state;
}
...
function renderPuzzle(component, scope = {}) {
  ...
  root.dataset.model = sourceId.model;
  root.dataset.source = sourceId.source;
  root.dataset.scene = sourceId.component;
  const key = `${sourceId.model}:${sourceId.component}:${sourceId.source}`;
...
function renderPuzzle3Frame(component, scope = {}) {
  if (!window.Puzzle3DFrameFixtures || !window.Puzzle3DFrameAssets || !window.Puzzle3Component) {
    throw new Error("Puzzle3 component assets are unavailable.");
  }
  const sourceId = runtimeViewportSourceId(component.source);
  const sceneName = sourceId.component;
  const source = sourceId.source;
  const key = `${sourceId.model}:${sceneName}:${source}`;
...
function puzzle3FrameFixture(sceneName, source = "board") {
  const modelFixture = window.Puzzle3DFrameFixtures?.[source];
  if (!modelFixture || typeof modelFixture !== "object") {
    throw new Error(`Puzzle3 model fixture is unavailable: ${source}`);
  }
  const fixture = JSON.parse(JSON.stringify(modelFixture));
  if (puzzle3PreviewSurface) {
  ...
  return mergePuzzle3SessionSnapshot(modelFixture, sessionSnapshot);
}
...
function setPuzzle3PreviewSurface(update = null) {
  ...
  const embed = componentEmbedActive();
...
function applyPuzzleStudioPreviewSurfaceUpdate(update = null) {
  ...
  const snapshot = puzzle3FrameFixture(
    surface.sceneName,
...
function shouldPostPuzzle3ComponentMessages() {
  return Boolean(componentEmbedActive() && window.parent && window.parent !== window);
}
...
function dispatchNextPresentationEvent() {
  ...
  dispatchingPresentationEvents = false;
  if (pendingPresentationContinuation) {
    completePendingPresentation();
    return;
  }
  drainQueuedModelInput();
}

async function completePendingPresentation() {
  if (completingPresentation) {
    return;
  }
  const token = pendingPresentationContinuation;
  if (!token) {
    return;
  }
  pendingPresentationContinuation = null;
  ...
  try {
    await postSessionAction({ kind: "presentation_complete", token });
  } catch (error) {
    if (currentState) {
  ...
  if (event.data?.type === "PuzzleStudioSetState") {
    ...
    try {
      if (standaloneRuntime && event.data.state) {
        ...
        window.parent?.postMessage({
          type: "PuzzleStudioPreviewRuntimeProgress",
          stage: "state-notified",
        }, "*");
      }
    } catch (error) {
      window.parent?.postMessage({
        type: "PuzzleStudioPreviewRuntimeError",
        label: "state injection failed",
...
if (puzzleBoot.editorPreview === true && standaloneRuntime && !initialPuzzle3PreviewSurface) {
  void standaloneRuntime.ensureInitialized()
