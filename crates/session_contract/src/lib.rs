...
/// Development-only fields projected independently from player presentation.
///
/// Runtime owners compose this with an already-resolved player snapshot. This
/// keeps editor inspection from becoming an intermediate representation on the
/// player path.
#[derive(Clone, Debug)]
pub struct RuntimeDevelopmentProjection {
    pub levels: BTreeMap<String, RuntimeLevelRecord>,
    pub solver_state: SolverStateSnapshot,
    pub selected_level_index: usize,
    pub inputs: Vec<RuntimeInputBinding>,
...
impl RuntimeDevelopmentSessionSnapshot {
    pub fn from_parts(
        player: RuntimeSessionSnapshot,
        development: RuntimeDevelopmentProjection,
    ) -> Self {
        Self {
            player,
            levels: development.levels,
            solver_state: development.solver_state,
            selected_level_index: development.selected_level_index,
            inputs: development.inputs,
            viewport_sources: development.viewport_sources,
        }
    }
}
