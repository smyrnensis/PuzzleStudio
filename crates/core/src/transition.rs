use crate::compiled_game::{CompiledGame, Pattern, RuleStep};
use crate::ids::{InputId, RuleId};
use crate::patch::{GridPatchError, Patch};
use crate::state::{Size2, State};
use puzzle_kernel::TransitionOutcome as KernelTransitionOutcome;
...
{
    let segment = crate::grid_transition::transition_program_segment_trace(
        game,
        program,
        state,
        Some(input),
    ...
    Ok(program_segment_trace(input, segment))
}
...
where
    F: FnMut(ProgramBoundarySnapshot<'_>) -> bool,
{
    let segment = crate::grid_transition::transition_program_continuation_segment_trace(
        game,
        program,
        continuation,
        state,
...
fn program_segment_trace(
    input: InputId,
    segment: crate::grid_transition::GridProgramSegmentTrace<2, Size2>,
) -> ProgramSegmentTrace {
    ProgramSegmentTrace {
        trace: StepTrace {
            input,
            next_state: segment.trace.next_state,
            cancelled: segment.trace.cancelled,
            commands: segment.trace.commands,
            fired_rules: segment.trace.fired_rules,
            patches: segment.trace.patches,
        },
        remaining_program: segment.remaining_program,
    }
}
...
fn run_program_transition(
    ...
    let outcome = crate::grid_transition::transition_program_outcome_with_local_frame(
        game, state, program, input, None,
    )?;
    Ok(StepTrace {
        input,
...
pub fn count_pattern_matches(game: &CompiledGame, state: &State, pattern: &Pattern) -> u32 {
    crate::grid_transition::count_pattern_matches(game, state, pattern)
}

pub fn has_pattern_match(game: &CompiledGame, state: &State, pattern: &Pattern) -> bool {
    crate::grid_transition::has_pattern_match(game, state, pattern)
}
